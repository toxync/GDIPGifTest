...
* TXYC_CaptionControl.h
  - 메인 다이얼로그와 캡션 탭 다이얼로그 간의 통신에 사용할 메시지 번호들 입니다.
* TXYC_GDIP.h
...
* TXYC_ThumbnailList.h
  ...
  - GifDasmDlg 다이얼로그에서 읽어들인 GIF 이미지 프레임들의 섬네일을 보여주는 용도로 사용됩니다.
* TXYC_FrameList.h
  - TXYC_ListBox 클래스를 상속 받은 사용자 정의 리스트 박스 클래스입니다.
  - GifAsmDlg 다이얼로그에서 읽어들인 이미지들의 섬네일을 보여주는 용도로 사용됩니다.
  - GIF 이미지 제작에 사용할 이미지 데이터를 보관하는 용도로 사용됩니다.
* TXYC_FrameThreadData.h
  - GifAsmDlg 다이얼로그에서 생성된 스레드가 사용할 데이터 구조체가 선언되어 있습니다.
  - GifAsmDlg 다이얼로그에서 사용할 벡터 자료구조의 자료형이 선언되어 있습니다.
* TXYC_CustomProgress.h
  - GDI+ 출력을 사용하는 사용자 정의 프로그레스 컨트롤 윈도우입니다.
* RectPositionFunc.h
  - 사각형 영역의 가로세로 비율을 유지하면서 다른 사각형 영역의 정중앙에 위치시키는 함수들입니다.
* OctreeQuantizer.h
  - GifAsmDlg 다이얼로그에서 사용할 octree quantization 알고리즘 기능을 제공하는 클래스입니다.
* GifDasmDlg.h
...
* ProgressDlg.h
  - GifAsmDlg 다이얼로그에서 스레드의 작업 진행 상태를 확인하기 위해 사용되는 다이얼로그입니다.
* PreviewDlg.h
  - GifAsmDlg 다이얼로그에서 특정 이미지의 8비트 컬러 변환 결과 이미지를 보여줄 때 사용되는 다이얼로그입니다.
...
* 메인 다이얼로그의 클라이언트 영역을 캡션 영역까지 확장한 다음 확장된 클라이언트 영역의 일부 공간을 논클라이언트 영역으로 사용합니다.
  - 메인 다이얼로그의 왼쪽, 오른쪽, 아래쪽 테두리는 기본 테두리를 그대로 사용했습니다.
  - 메인 다이얼로그의 클라이언트 영역을 캡션 영역까지 확장했기 때문에 기본 위쪽 테두리가 없어져 버려서 위쪽 테두리를 자체적으로 구현했습니다.
  - 위쪽 테두리의 영역을 보기 쉽도록 빨간색 사각형으로 표시했습니다.
* 캡션 탭 기능을 제외한 나머지 기능들은 아래와 같이 일반적인 윈도우의 논클라이언트 영역 기능과 동일합니다.
...
* 프레임 저장 스플릿 버튼으로 프레임 저장 모드를 변경할 수 있으며, 사용할 수 있는 프레임 저장 모드는 아래와 같습니다.
...
* 분해된 GIF 이미지의 모든 프레임들을 저장하면 읽어들인 GIF 이미지의 파일 이름과 이름이 동일한 폴더 안에 분해된 GIF 이미지의 모든 프레임들이 저장됩니다.
...
* 이미지를 읽어들이면 원본 이미지와 원본 이미지 기반의 프레임 이미지가 저장된 리스트 박스 항목이 만들어집니다.(프레임 이미지는 기준 프레임과 동일한 크기로 생성됩니다.)
...
* 프레임 지연 시간이 변경될 때마다 변경된 프레임 지연 시간의 실제 프레임 지연 시간이 실제 프레임 지연 시간 에디트 컨트롤에 자동으로 반영됩니다.
...
* '선택된 프레임의 이동 거리' 텍스트 아래에 있는 스플릿 버튼을 통해 현재 선택된 프레임의 1회 이동 거리를 설정할 수 있으며, 설정가능한 이동거리는 아래와 같습니다.
  - 1
  - 5
  - 10
  - 마지막 위치로
