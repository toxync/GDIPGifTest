...
GpImage* TXYC_GDIP::CreateGif(const wchar_t* ap_path, GpBitmap** ap_frame_list, UINT a_frame_cnt, UINT a_delay)
{
	CLSID cls_id;
	...
	long* p_value = NULL;
	GetEncoderClsid(L"image/gif", &cls_id);
	...
	p_value = new long[a_frame_cnt];
	for (int i = 0; i < a_frame_cnt; ++i)
	{
		p_value[i] = a_delay;
	}
	property_item.id = PropertyTagFrameDelay;
	property_item.length = sizeof(long) * a_frame_cnt;
	property_item.type = PropertyTagTypeLong;
	property_item.value = p_value;
	GdipSetPropertyItem(p_gif_image, &property_item);
	delete[] p_value;
	// 미리 설정해둔 저장 위치에 GIF 이미지 파일 생성
	GdipSaveImageToFile(p_gif_image, ap_path, &cls_id, &encoder_param);
	// GIF 이미지에 두 번째 프레임부터 마지막 프레임까지 삽입
	e_value = EncoderValueFrameDimensionTime;
	for (int i = 1; i < a_frame_cnt; ++i)
	{
		GdipSaveAddImage(p_gif_image, *(ap_frame_list + i), &encoder_param);
	}
...
UINT TXYC_GDIP::GetGifData(GpImage* ap_gif, GUID** ap_id_list, PropertyItem** ap_item, UINT** ap_delay_list)
{
	// 첫 프레임 그룹에 포함된 프레임 수, 프레임 정보의 크기, 읽어들인 GIF 이미지에서 추출한 프레임 그룹 개수
	UINT frame_cnt = 0, total_size = 0, dim_cnt = 0;
	// 프레임 배열의 수를 얻어옴((GIF 이미지의 전체 프레임 수) = (프레임 인덱스) * (애니메이션 개수))
	GdipImageGetFrameDimensionsCount(ap_gif, &dim_cnt);
	// 프레임 배열에 대한 GUID를 저장할 메모리를 할당함
	*ap_id_list = new GUID[dim_cnt];
	// 프레임 그룹에 대한 GUID를 얻음
	GdipImageGetFrameDimensionsList(ap_gif, *ap_id_list, dim_cnt);
	// 현재 프레임 그룹에 대한 프레임 개수를 얻음
	GdipImageGetFrameCount(ap_gif, *ap_id_list, &frame_cnt);
	// 프레임간의 지연 시간이 포함되어 있는 정보의 크기를 얻음
	GdipGetPropertyItemSize(ap_gif, PropertyTagFrameDelay, &total_size);
	// 지연 시간 정보를 저장하기 위한 메모리를 할당함
	*ap_item = (PropertyItem*)malloc(total_size);
	// 지연 시간 정보를 얻어옴
	GdipGetPropertyItem(ap_gif, PropertyTagFrameDelay, total_size, *ap_item);
	// 프레임 지연 시간 목록을 얻어옴
	*ap_delay_list = (UINT*)(*ap_item)->value;
	return frame_cnt;
}
